          ...
          {article?.length > 0 ? (
            <>
              {article.map((item: any, index: number) => (
                <div
                  key={index}
                  className="p-5 border bg-white  hover:shadow-md hover:scale-[1.005] duration-200 rounded-xl grid grid-cols-11 gap-5 items-center"
                >
                  <p className="pl-5 font-semibold">{item?.id}</p>
                  <p className="col-span-4 line-clamp-2">{item?.title}</p>
                  <p className="col-span-2 line-clamp-1">{item?.type}</p>
